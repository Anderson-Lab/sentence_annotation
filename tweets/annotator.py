...
def get_random_tweet(df):
    ...
    new_df = df[df['mask'].str.len() == 0].reset_index()
    return new_df.sample(n=1)


def on_create_mask(row, mask, mask_file_name):
    global original_df
    global mask_df
    global unseen_df
    row['mask'] = [mask]
...
def display_buttons(row, create_mask_callback, mask_file_name, sentiments=['"negative"', '"positive"', '"neutral"']):
    ...
    create_button = Button(description='Create Mask', layout=Layout(
        width='auto', height='auto', padding='2px 15px 2px 15px'))
    ...
    create_wrapper = HBox([create_button], layout=Layout(
...
def reset_display(mask_file_name):
    global original_df
    global mask_df
    global unseen_df
    unseen_df = remove_already_seen(original_df, mask_df)
    tweet = get_random_tweet(unseen_df)
    display_buttons(tweet, on_create_mask, mask_file_name)


def run_annotator(original_file_name, mask_file_name):
    global original_df
    global mask_df
    global unseen_df
